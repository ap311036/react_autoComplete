...
export default class Module extends Component {
    ...
    cloneDataArray () { // 複製一個DATA出來操作 不要動到props
        let dataHasDataInedx = JSON.parse(JSON.stringify([...this.props.data])).sort((a, b) => a.txt.length - b.txt.length);
        dataHasDataInedx.map((item, i) => {
            item.dataIndex = i;
            return item;
        });
        return this.lightingMatchWord(dataHasDataInedx);
    }
